...
- **Every English change ships with its Urdu, in the same edit.** Not "later", not a follow-up task — the same commit. A page where one language says more than the other is worse than one that says less in both, because nobody notices: it renders fine, and only an Urdu reader ever sees the gap. This session found two promises that had drifted exactly that way (`PriceReceipt` lost "if the person does not feel right", `TeamSection` lost "before anyone reaches your door"), and then I did it again while trimming an area page. If a string genuinely needs no Urdu — a JSON-LD value, a canonical URL, a machine error code — that is a decision worth a one-line comment, not a silent omission.
- **Replace whole bilingual spans, never patch them mid-sentence.** Urdu is RTL and unfamiliar to most people editing this repo, so a substring replace that reads fine in a diff can leave a grammatically broken sentence that nobody spots. Rewrite the entire `data-ur` span to match the new English. Then read the rendered page, not the diff.
- **Internal rules stay internal.** "We have not measured a response time, so do not invent one" is a rule for us; printing it on the site is the site narrating its own honesty instead of being useful. The customer wants the fact — "we agree the arrival time with you on the call" — not a statement about our editorial standards. Same for "we would rather say that here than at your door" and "we will not pretend otherwise". State what happens; skip the commentary about how honest we are being. (A promise made *to the customer* — "we will not send a nurse where an attendant is enough" — is different and stays: that is a commitment, not self-praise.)
- **Verify in the rendered HTML, not the source.** Several real bugs this session were invisible in the code and obvious in the output: the sticky bar's Urdu that never rendered, five pages with no H1, the sitemap silently missing four pages. `npm run build`, then grep `.next/server/app/**/*.html`. Code-correct is not done; rendered-correct is done.
- **Don't refuse out loud unless refusing is necessary.** Not promising something is enough; an explicit "no" turns work we *could* take into work we have publicly declined. Describe what the caregiver comes for, then invite the ask on the first call. Newborn care is the worked example: it isn't usual, but it could be arranged, so the page says what the caregiver is there for and asks the family to raise it — it does not say "no". **Necessary refusals stay**, and they are narrow: things we genuinely cannot do safely (ventilator and tracheostomy care, Ledger #16), things outside a nurse's scope (we do not diagnose or prescribe), medical guidance that belongs to the treating doctor (no symptom lists), capabilities a family would otherwise assume (no cameras or GPS on `/care-from-abroad`), and the hospital no-affiliation line, which is legal cover. Everything else: say what we do, and leave the door open.
...
| `components/home/` | `LandingRoot`, `Hero`, `HowItWorks`, `PriceReceipt`, `ServicesSection`, `VerifiedCard`, `HomeFAQ`, `FounderNote`, `CtaBanner`, `LeadFormD6`, `ConfirmGreeting` |
